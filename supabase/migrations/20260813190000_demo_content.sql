-- =====================================================================
-- Demo content — promoted from seed.sql (2026-08-13) so it reaches
-- PRODUCTION and examples can be shown there: six topics, two dozen
-- live questions across all three templates, three active batches with
-- queues, twelve fake contestants with deliberate answer patterns, the
-- pod lead's links with pod attribution, and the Wave-2 proposal rows.
--
-- Everything here was previously replayed locally by `supabase db
-- reset`; it now lives in a migration so `supabase db push` installs
-- the same demoable state in production exactly once. seed.sql keeps
-- only what must stay local: the known-password logins.
--
-- SECURITY — the two demo staff users below (Sam the pod lead, Ryan
-- the DOL) exist because batch_links.owner_id and the proposal rows'
-- author_id / reviewed_by reference them. In production they are
-- created with UNGUESSABLE random passwords and are not meant to be
-- signed into; seed.sql resets them to 'password' for local dev only.
-- To actually log in as one in production, set a password by hand in
-- the Supabase dashboard (Auth → Users). No admin user is created
-- here — production's admin comes from `pnpm --dir my-app
-- bootstrap:admin`, same as before.
--
-- Fixed UUIDs throughout so the cross-references stay readable:
--   staff     ...a000-000000000002 (Sam) · 000000000003 (Ryan)
--   topics    ...a000-000000000101 … 106
--   questions ...a000-000000000201 … 227
--   batches   ...a000-000000000301 … 303
--   contestants ...a000-000000000401 … 412
--   batch links ...a000-000000000501 … 504
--
-- No principles here — the rubric is installed by migration
-- 20260810130000_replace_placeholder_principles.sql; this file only
-- links against it.
-- =====================================================================

-- ---------------------------------------------------------------------
-- Demo staff: Sam (pod lead) and Ryan (DOL). Random passwords —
-- see the SECURITY note above. Identities are inserted so the rows
-- look like every other email user to GoTrue.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Wave 1 (pod leads): Sam's links on every master batch, and pod
-- attribution for some of the fake contestants.
--
